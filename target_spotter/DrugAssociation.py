...
class DrugAssociation:
    """
    Class to fit and predict drug sensitivities from splicing dependency profiles.
    ...
    def fit(self, drug_response, splicing_dependency, growth_rates=None, mapping=None):
        """
        Fit each exon's predicted splicing depenendency to each drug's sensitivity profile across cancer cell lines with the following linear model based on [Gonçalves (2020)](https://doi.org/10.15252/msb.20199405):
        
            $$log(IC50) = 1 + SplicingDependency + GrowthRate +\epsilon$$
        
        Parameters
        ----------
        drug_response : pd.DataFrame
            Drug sensitivity IC50 measurements. It is assumed that you are using GDSC tables from the
            "sanger-dose-response.csv" file publicly available at https://depmap.org/portal/download/all/?release=Sanger+GDSC1+and+GDSC2&file=sanger-dose-response.csv.
            We need to have the following columns:
            - IC50_PUBLISHED: present in the original file.
            - ARXSPAN_ID: depmap cell line identifier; present in the original file.
            - ID: unique drug identifier generated by combining DRUG_ID and MAX_CONC columns from the original file.
        
        splicing_dependency : pd.DataFrame of shape (n_cancer_driver_exons, n_samples)
            Average or median splicing dependencies. Obtained running `SplicingDependency.predict()` on your transcriptomic data.
        
        growth_rates :  pd.DataFrame, default=None
            With DepMap cell line identifiers as index and a column named "growth_rate". If growth_rates is None, we calculate them using [Gonçalves (2020)](https://doi.org/10.15252/msb.20199405)'s method based on using the first principal component of the drug sensitivities as growth rate.
        ...
        Attributes
        ----------
        model_summaries_ : pd.DataFrame
            Exon-drug level summary statistics of the training.
        
        growth_rates_ : pd.DataFrame
            Growth rates used in fitting.
        """
        # prepare
    ...
    def predict(
        ...
        growth_rates :  pd.DataFrame, default=None
            With corresponding sample identifiers in `splicing_dependency`. If `growth_rates` is None, we infer them by KNN imputation from the growth rates inferred for the cancer cell lines in the training based on their splicing dependency profiles.
        
        model_summaries : pd.DataFrame, default=None
            Exon-drug level summary statistics of the training. If `model_summaries` is None, we use the default model summaries generated during training in our publication.

        fitted_growth_rates : pd.DataFrame, default=None
            Sample-level growth rates used during training. If `fitted_growth_rates` is None, we use the default growth rates generated during training in our publication.
        
        splicing_dependency : pd.DataFrame of shape (n_cancer_driver_exons, n_samples), default=None
            Average or median splicing dependencies obtained running `SplicingDependency.predict()` with transcriptomic data used during training. If `splicing_dependency` is None, we use the default splicing dependencies generated during training in our publication.
        
        dataset : {"GDSC1", "GDSC2"}, default="GDSC1"
            GDSC dataset to use. These screens use different technologies and were therefore considered separately. This is only relevant if model_summaries=None as we will use the models fitted in our publication.
        
        Attributes
        ----------
        splicing_dependency_ : pd.DataFrame
            Original splicing dependency provided as parameter.
            
        growth_rates_ : pd.DataFrame
            Growth rates used to make prediction.
            
        model_summaries_ : pd.DataFrame
            Model summaries used.
            
        prep_splicing_dependency_ : pd.DataFrame
            Preprocessed splicing dependencies.
        
        prep_growth_rates_ : pd.DataFrame
            Preprocessed growth rates.
            
        prep_model_summaries_ : pd.DataFrame
            Preprocessed model summaries.
            
        fitted_growth_rates_ : pd.DataFrame
            Growth rates from model fitting.
        
        fitted_spldep_ : pd.DataFrame
            Splicing dependencies from model fitting.
        
        dataset_ : str
            GDSC dataset selected.
        
        Returns
        -------
        drug_estimates_ : pd.DataFrame of shape (n_drugs, n_samples)
            Inferred drug log-IC50s.
            
        full_estimates_ : pd.DataFrame of shape (n_drugs*n_cancer_driver_exons, n_samples)
            Inferred contribution of each exon to each drug's log-IC50.
